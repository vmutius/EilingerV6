                ...
                @forelse ($users as $user)
                    @if ($user->sendApplications->count() <2)
                        <tr>
                            ...
                            @forelse ($user->sendApplications as $application)
                                <td>
                                    <a href="{{ route('admin_antrag', ['application_id' => $application->id, 'locale'=> app()->getLocale()]) }}">{{ $application->name }}</a>
                                </td>
                                ...
                                <td><span
                                        class="badge text-bg-{{ $application->appl_status_context }}">{{ __('application.status.' .$application->appl_status->name) }}</span>
                                </td>
                            @empty
                                <td></td>
                                <td></td>
                    ...
                    @else
                        <tr>
                            ...
                            <td rowspan="{{ $user->sendApplications->count() }}">{{ $user->created_at ? $user->created_at->format('d.m.Y H:i') :null }}</td>
